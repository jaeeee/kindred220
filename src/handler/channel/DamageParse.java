        ...
        for (final AttackPair oned : attack.allDamage) {
            ...
            if (monster != null) {
                ...
                if (totDamageToOneMonster > 0) {
                        ...
                        if (player.getSkillLevel(3101009) > 0) {
                                if (attack.skill > 0) {
                                    if (player.QuiverCallCount == 2 && player.getQuiverArrowCount(1) <= 0 && player.getQuiverArrowCount(1) > -10) {
                                        ...
                                        player.send(MainPacketCreator.QuiverKartrige(player, 1, player.getQuiverArrowCount(1), true));
                                        ...
                                        player.send(MainPacketCreator.QuiverKartrige(player, 2, player.getQuiverArrowCount(2), true));
                                        if (Randomizer.nextInt(100) < 90) {
                                            monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, 1), SkillFactory.getSkill(3101009), null, false), true, (long) 5 * 1000, false);
                                        }
                                        ...
                                        player.send(MainPacketCreator.QuiverKartrige(player, 3, player.getQuiverArrowCount(3), true));
                                    if (player.getBuffedValue(BuffStats.ADVANCED_QUIVER) != null) {
                    ...
                    if (player.getBuffedValue(BuffStats.VAMPIRIC_TOUCH) != null && totDamageToOneMonster <= 999999 && totDamageToOneMonster > 0) { //뱀피릭 터치
                        ...
                        if (totDamageToOneMonster > 0) {
                            if (player.getBuffedValue(BuffStats.BLIND) != null) {
                                if (player.getJob() >= 3311 && player.getJob() <= 3312) {
                                    ...
                                    if (eff.makeChanceResult()) {
                                        monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.DARKNESS, eff.getX()), skill, null, false);
                                        monster.applyStatus(player, monsterStatusEffect, false, eff.getY() * 1000, false);
                                        MonsterStatusEffect monsterStatusEffect2 = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.WDEF, -eff.getSkillStats().getStats("v")), skill, null, false);
                                        monster.applyStatus(player, monsterStatusEffect2, false, eff.getY() * 1000, false);
                                        MonsterStatusEffect monsterStatusEffect3 = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.MDEF, -eff.getSkillStats().getStats("w")), skill, null, false);
                                ...
                                if (eff.makeChanceResult()) {
                                    monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.SPEED, eff.getX()), skill, null, false);
                                    monster.applyStatus(player, monsterStatusEffect, false, eff.getY() * 1000, false);
                                }
                ...
                /* 더 시드 소스 시작 */
                if ((monster.getId() == 9309044 || monster.getId() == 9309045) && !player.getMap().getnewTheSidClear(2)) {
                    float a = 0;
                    long b = 0;
                    String c = null;
                    if(player.getMap().getDamage() < 5000000000L) {
                      ...
                      player.getMap().broadcastMessage(UIPacket.greenShowInfo("누적데미지 " + c + " / 50억" ));
                         ...
                         player.getMap().broadcastMessage(TheSidPacket.TheSidMessage("잠시뒤에 또다른 몬스터가 나올거야!! 긴장을 늦추지 말아줘!", 3000));
...
//                        player.send(MainPacketCreator.multikill(mobs, comboexp));
            ...
            if (attack.skill != 0) {
                player.changeMap(player.getClient().getChannelServer().getMapFactory().getMap(101050010), player.getClient().getChannelServer().getMapFactory().getMap(101050010).getPortalSP().get(0));
    ...
    public static void applyAttackMagic(AttackInfo attack, ISkill theSkill, MapleCharacter player, SkillStatEffect effect) {
        ...
        Element element = player.getBuffedValue(BuffStats.ELEMENT_RESET) != null ? Element.NEUTRAL : theSkill.getElement();
...
//                             player.send(MainPacketCreator.multikill(mobs, comboexp));
                    ...
                    /*  더 시드 소스 끝 */
                    if (player.getBuffedValue(BuffStats.COMBO_DRAIN, 32101004) != null) { //콤보 드레인
                        ISkill skill = SkillFactory.getSkill(32101004);
                        player.addHP((Math.min(totDamage / 5, (totDamage * skill.getEffect(player.getSkillLevel(skill)).getX()) / 100)));
                    }
                    if (eaterLevel > 0) {
        ...
        if (attack.skill != 2301002) {
            ...
            if (attack.skill != 0) {
                    player.changeMap(player.getClient().getChannelServer().getMapFactory().getMap(100000003), player.getClient().getChannelServer().getMapFactory().getMap(100000003).getPortalSP().get(0));
                    player.Message(6, "마을에서는 스킬을 사용하시면 안됩니다");
               }
                }
    }
        ...
        for (final Pair<Integer, Boolean> eachde : oned.attack) {
            ...
            if (s.makeChanceResult()) {
                EtcTimer.getInstance().schedule(new Runnable() {
                    @Override
                    public void run() {
                        player.getMap().spawnMesoDrop(1, new Point((int) (mob.getPosition().getX() + Randomizer.nextInt(100) - 50), (int) (mob.getPosition().getY())), mob, player, true, (byte) 0);
                    }
                }, 100L);
            }
        }
    ...
    public static AttackInfo parseDmgAsura(ReadingMaple rh) {
        ...
        rh.skip(7); // ORDER [4] bytes on v.79, [4] bytes on v.80, [1] byte on v.82, [6] byte on 160, [7] byte on 169
