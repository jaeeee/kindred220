...
public class MapleCharacterStat {
   ...
   public final void setMaxMp(final int mp) {
    ...
    public void recalcLocalStats() {
        ...
        setHandling.clear();
	MapleCharacter chra = chr.get(); // 이게 눌이 되서 오류가 뜨는거거든여
        //수납의 달인
...
//                }
        ...
        /* 어디다가 추가를 해야될질 모르겠어서 그냥 여기에 추가... 문제생기면 삭제/이동 부탁..*/
...
//        }
    ...
    public final void calculateStatAttack(MapleCharacter chr) {
        ...
        if (jobtype == null) {
        //    System.err.println("[ X ] 스탯 공격력 계산 중 오류가 발생. - 알 수 없는 직업 코드입니다. : " + chr.getJob());
            return;
        }
	...
	if (chr.getBuffedValue(BuffStats.ROLL_OF_DICE) != null) {
        ...
        for (Entry<ISkill, SkillEntry> se : chr.getSkills().entrySet()) {
            try {
               ...
               // if (se.getKey() != null)
                 //   System.err.println("[ X ] 스탯 공격력을 계산하던 중 오류 발생.  스킬 : "+se.getKey().getId());
            }
        }
    }
