...
public class MapleMap {
	...
	private short decHP = 0, createMobInterval = 3500; // default create mob
														// interval = 9000
	...
	public MapleMap(final int mapid, final int channel, final int returnMapId, final float monsterRate) {
		...
		EnumMap<MapleMapObjectType, LinkedHashMap<Integer, MapleMapObject>> mapobj = new EnumMap<MapleMapObjectType, LinkedHashMap<Integer, MapleMapObject>>(
	...
	private void spawnMapObjectList(final List<ObtacleAtom> mapobjects, final DelayedPacketCreation packetbakery) {
		...
		MapleCharacter chr;
		while (ltr.hasNext()) {
			// for (MapleMapObject obj :
			// this.mapobjects.get(MapleMapObjectType.PLAYER).values()) {
			chr = ltr.next();
	...
	public final List<MapleMist> getMistInRange(final Point from, final double rangeSq,
	...
	public final List<MapleSummon> getSummonInRange(final Point from, final double rangeSq,
			final List<MapleMapObjectType> MapObject_types) {
		final List<MapleMapObject> ret = new LinkedList<MapleMapObject>();
	...
	public final List<MapleRune> getRuneInRange(final Point from, final double rangeSq,
			final List<MapleMapObjectType> MapObject_types) {
	...
	private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
		...
		final int mobpos = mob.getTruePosition().x, cmServerrate = ChannelServer.getInstance(channel).getMesoRate(),
				chServerrate = ChannelServer.getInstance(channel).getDropRate(), caServerrate = 10;
		Item idrop;
		Item te;
		byte d = 1;
		...
		for (final MonsterDropEntry de : dropEntry) {
			...
			if ((Randomizer.nextInt(999999) < de.chance * chServerrate * (showdown / 100.0D))
					&& ((maxdrop == -1) || (d < maxdrop + 1))) {
				...
				if (de.itemId == 0) {
					...
					if (mesos > 0) {
						spawnMobMesoDrop((int) (mesos * cmServerrate), calcDropPos(pos, mob.getTruePosition()), mob,
								chr, false, droptype);
						mesoDropped = true;
					}
				} else {
		...
		for (final MonsterGlobalDropEntry de : globalEntry) {
			// if (getChannel() == 1 || getChannel() == 10 || getChannel() == 11
			// || getChannel() == 12
			// || getChannel() == 13 || getChannel() == 14 || getChannel() == 15
			// || getChannel() == 16
			// || getChannel() == 17 || getChannel() == 18 || getChannel() ==
			// 19) { // buffed
			...
			if (Randomizer.nextInt(999999) < de.chance) {
				if (de.itemId != 0) {
					...
					if ((de.itemId == 4001513
							&& !(mob.getStats().getLevel() >= 105 && mob.getStats().getLevel() <= 114))
							|| (de.itemId == 4001515
									&& !(mob.getStats().getLevel() >= 115 && mob.getStats().getLevel() <= 159))
							|| (de.itemId == 4001521
									&& !(mob.getStats().getLevel() >= 160 && mob.getStats().getLevel() <= 250))) {
						return;
					}
					for (byte cid : KindredConstants.BUFFED_CHANNELS) {
						if (getChannel() != cid) {
							for (int drp : KindredConstants.BUFFED_DROPS) {
								if (de.itemId == drp) {
									return;
								}
								break;
	...
	public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops,
		...
		if (chr.getBuffedValue(BuffStats.RIVIBE) != null) {
			...
			if (effect.makeChanceResult()) {
				final MapleSummon reaper = new MapleSummon(chr, 32111006, effect.getX(), monster.getPosition(),
		...
		if (monster.getBuffToGive() > -1) {
			...
			for (final MapleMapObject mmo : characters) {
				...
				if (c.isAlive()) {
					...
					case 8820001:
					case 8820101:
						c.getClient().getSession().write(MainPacketCreator.showOwnBuffEffect(buffid, 11)); // HT
																											// nine
																											// spirit
						broadcastMessage(c, MainPacketCreator.showBuffeffect(c.getId(), c.getLevel(), buffid, 11),
		...
		final int mobid = monster.getId();
		 ...
		 if (mobid == 8810018) { //Horntail
			...
			for (final MapleMapObject mmo : getAllMonster()) {
				MapleMonster mons = (MapleMonster) mmo;
				if (mons.getId() != 8820100 && mons.getId() != 8820101 && mons.getObjectId() != monster.getObjectId()
						&& mons.isAlive() && mons.getLinkOid() == monster.getObjectId()) {
					killMonster(mons, chr, false, false, animation);
				}
			}
		...
		if (monster.getId() == 9300580 || monster.getId() == 9300581 || monster.getId() == 9300582
				|| monster.getId() == 9300583 || monster.getId() == 9300584 || monster.getId() == 9300585
				|| monster.getId() == 9300586 || monster.getId() == 9300587) {
			미스틱혼테일++;
		}

		if (monster.getId() == 8810102 || monster.getId() == 8810103 || monster.getId() == 8810104
				|| monster.getId() == 8810105 || monster.getId() == 8810106 || monster.getId() == 8810107
				|| monster.getId() == 8810108 || monster.getId() == 8810109) {
			카오스혼테일++;
		}
		...
		if (monster.getId() == 8910001 || monster.getId() == 8900102 || monster.getId() == 8920103
			...
			int i = monster.getId() == 8930100 ? 7 + Randomizer.nextInt(13)
					: monster.getId() == 8920103 ? 5 + Randomizer.nextInt(4)
							: monster.getId() == 8900102 ? 3 + Randomizer.nextInt(4) : 6 + Randomizer.nextInt(9);
	...
	public MapleMonster makeEliteBoss(final MapleMonster sourcemob, final MapleMonster sourceboss) {
		...
		ostats.setOHp(sourcemob.getMobMaxHp() * 75); // set hp times 75
		...
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			...
			public final void sendPackets(MapleClient c) {
				c.getSession().write(MobPacket.spawnMonster(monster, -2, 0, oid,
						GameConstants.isAswanMap(c.getPlayer().getMapId()))); // TODO
																				// effect
			}
		}, null);
	...
	public final void spawnMonster(final MapleMonster monster, final int spawnType, final int effect) {
		...
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			public final void sendPackets(MapleClient c) {
				if (c != null) {
					c.getSession().write(MobPacket.spawnMonster(monster, spawnType, effect, 0,
	...
	public final void spawnObatcleAtom(final List<ObtacleAtom> atom) {
		 ...
		 * MapTimer.getInstance().schedule(new Runnable() {
		 * 
		 * @Override public void run() { for (ObtacleAtom dd : atom) {
		 * broadcastMessage(BossPackets.removeObtacleAtomBomb(dd.getObjectId()))
		 * ; } removeMapObjectList(atom); } }, 22000);
		 */
	}
			...
			poisonSchedule = tMan.register(new Runnable() {
				...
				public void run() {
					for (final MapleMapObject mo : getAllPlayer()) {
						...
						boolean contain = getMapObjectsInRect(mist.getBox(),
								Collections.singletonList(MapleMapObjectType.PLAYER)).contains(mo);
	...
	public final void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final IItem item,
		...
		spawnAndAddRangedMapObject(drop, new DelayedPacketCreation() {
			...
			public void sendPackets(MapleClient c) {
				c.getSession()
						.write(MainPacketCreator.dropItemFromMapObject(drop, dropper.getPosition(), droppos, (byte) 1));
			}
		}, null);
	...
	public final void addPlayer(final MapleCharacter chr) {
		...
		if (mapid == 109050001) { // Event map exit
		...
		for (int i = 0; i < 3; ++i) {
			if (chr.getPet(i) != null) {
				chr.getPet(i).setPos(chr.getPosition()); // update pet coordinates
		...
		if (chr.getAndroid() != null) { // Set
			chr.getAndroid().setPosition(chr.getPosition()); // update android coordinates
		...
		if (chr.getParty() != null) {
			...
			chr.getClient().getSession().write(MainPacketCreator.updateParty(chr.getClient().getChannel(),
			...
			for (MapleSummon summon : chr.getSummons().values()) {
				if (!summon.isPuppet() && !summon.isStaticSummon()) {
					...
					spawnSummon(summon, false,
							SkillFactory.getSkill(summon.getSkill()).getEffect(summon.getSkillLevel()).getDuration());
				}
			}
		}
		...
		if (chr.getEventInstance() != null && chr.getEventInstance().isTimerStarted()
				&& !chr.getEventInstance().isCleared()) {
			chr.getClient().getSession()
					.write(MainPacketCreator.getClock((int) (chr.getEventInstance().getTimeLeft() / 1000)));
		}

		if (chr.getEventInstance() != null && chr.getEventInstance().isUsingAchievementRatio()
				&& !chr.getEventInstance().isCleared()) {
		...
		if (hasClock()) {
			...
			chr.getClient().getSession().write((MainPacketCreator.getClockTime(cal.get(Calendar.HOUR_OF_DAY),
		...
		if (GameConstants.isEvan(chr.getJob()) && chr.getJob() >= 2200
				&& chr.getBuffedValue(BuffStats.MONSTER_RIDING) == null) {
		...
		for (BuffStats check : GameConstants.getBroadcastBuffs()) {
			if (chr.getBuffedValue(check) != null) {
				...
				broadcastMessage(chr,
						MainPacketCreator.giveForeignBuff(chr.getId(),
		...
		for (MapleDiseaseValueHolder hdvh : chr.getAllDiseases()) {
			for (Pair<DiseaseStats, Integer> p : GameConstants.getBroadcastDebuffs()) {
				if (hdvh.disease == p.getLeft()) {
					broadcastMessage(MainPacketCreator.giveForeignDebuff(chr.getId(), p.getLeft(),
		...
		if (chr.getMapId() == 105200410) {
			chr.send(MainPacketCreator.getNPCTalk(2007, (byte) 0, "What? What is that monster over there?\r\n(Let us talk to it)", "00 01",
					(byte) 2));
	...
	private final void broadcastMessage(final MapleCharacter source, final Packet packet, final double rangeSq,
	...
	public void setMapTimer(long time) {
		try {
			PreparedStatement ps = MYSQL.getConnection()
					.prepareStatement("DELETE FROM bosscooltime WHERE map = ? AND channel = ?");
			ps.setInt(1, mapid);
			ps.setInt(2, channel);
			ps.executeUpdate();
			...
			System.err.println("[Error] Failed to save boss cooldown to DB.");
	...
	public void startCatch() {
		if (catchstart == null) {
			...
			catchstart = MapTimer.getInstance().schedule(new Runnable() {
				...
				public void run() {
					broadcastMessage(
							MainPacketCreator.serverNotice(1, "The game is over! It has been 2 minutes. Everyone will be moved to the rewards map."));
	...
	public void startTheSid(int type) { //tower of oz
		if (type == 1) {
			broadcastMessage(MainPacketCreator.showEffect("defense/count"));
			broadcastMessage(TheSidPacket.TheSidMessage("Monsters will come out soon. Prepare for battle!", 3000));
			EtcTimer tMan = EtcTimer.getInstance();
			tMan.schedule(new Runnable() {
				@Override
				public void run() {
					broadcastMessage(MainPacketCreator.showEffect("defense/wave/1"));
					broadcastMessage(TheSidPacket.TheSidMessage("First of all, i want you to defeat all the ancient slimes!", 3000));
					for (int i = 0; i < 30; i++) {
			...
			tMan.schedule(new Runnable() {
				...
				public void run() {
					broadcastMessage(MainPacketCreator.showEffect("defense/count"));
					broadcastMessage(TheSidPacket.TheSidMessage("Crush the ancient turtles!", 3000));
				}
			}, 3000);
			...
			tMan.schedule(new Runnable() {
				...
				public void run() {
					broadcastMessage(MainPacketCreator.showEffect("defense/count"));
					broadcastMessage(TheSidPacket.TheSidMessage("do 5billion damage to the Volcanoes!", 3000));
				}
			}, 3000);
			EtcTimer tMan1 = EtcTimer.getInstance();
			tMan1.schedule(new Runnable() {
			...
			tMan.schedule(new Runnable() {
				...
				public void run() {
					broadcastMessage(MainPacketCreator.showEffect("defense/count"));
					broadcastMessage(TheSidPacket.TheSidMessage("Watch out for the spider!", 3000));
				}
			}, 3000);
			EtcTimer tMan1 = EtcTimer.getInstance();
			tMan1.schedule(new Runnable() {
			...
			tMan.schedule(new Runnable() {
				...
				public void run() {
					broadcastMessage(MainPacketCreator.showEffect("defense/count"));
					broadcastMessage(TheSidPacket.TheSidMessage("One of the officiers, the Scarecrow is coming out!", 3000));
				}
			}, 2000);
			EtcTimer tMan1 = EtcTimer.getInstance();
			tMan1.schedule(new Runnable() {
			...
			tMan.schedule(new Runnable() {
				...
				public void run() {
					broadcastMessage(MainPacketCreator.showEffect("defense/count"));
					broadcastMessage(TheSidPacket.TheSidMessage("Another officier, the lion will hunt you!", 3000));
				}
			}, 2000);
			EtcTimer tMan1 = EtcTimer.getInstance();
			tMan1.schedule(new Runnable() {
			...
			tMan.schedule(new Runnable() {
				...
				public void run() {
					broadcastMessage(MainPacketCreator.showEffect("defense/count"));
					broadcastMessage(TheSidPacket.TheSidMessage("The final boss, Dorothy will come out!", 3000));
				}
			}, 2000);
			EtcTimer tMan1 = EtcTimer.getInstance();
			tMan1.schedule(new Runnable() {
	...
	public void TheSidMain(MapleMonster monster) {
		if (monster.getId() == 9309205) {
			broadcastMessage(MainPacketCreator.showEffect("Gstar/clearS"));
			broadcastMessage(TheSidPacket.TheSidMessage("It is great to kill the Scarecrow!", 3000));
			this.spawnNpc(2540005, new Point(-461, 108));
		} else if (monster.getId() == 9309200) {
			broadcastMessage(MainPacketCreator.showEffect("Gstar/clearS"));
			broadcastMessage(TheSidPacket.TheSidMessage("It is great to terrify the Lion!", 3000));
			spawnNpc(2540005, new Point(1273, 155));
		} else if (monster.getId() == 9309203) {
			broadcastMessage(MainPacketCreator.showEffect("Gstar/clearS"));
			broadcastMessage(TheSidPacket.TheSidMessage("It's great to have a ScareCrow!", 3000));
			spawnNpc(2540005, new Point(1273, 155));
		} else if (monster.getId() == 9309207) {
			broadcastMessage(MainPacketCreator.showEffect("Gstar/clearS"));
			broadcastMessage(TheSidPacket.TheSidMessage("Congratulations on saving the Seed!", 3000));
			spawnNpc(2540005, new Point(77, 185));
		} else if (!newthesid[0]) {
			슬라임카운트++;
			if (슬라임카운트 == 30 || 슬라임카운트 == 60 || 슬라임카운트 == 90 || 슬라임카운트 == 120 || 슬라임카운트 == 150 || 슬라임카운트 == 180
					|| 슬라임카운트 == 210 || 슬라임카운트 == 240 || 슬라임카운트 == 270) {
				for (int i = 0; i < 30; i++) {
					MapleMonster mob = MapleLifeProvider.getMonster(Randomizer.rand(9309047, 9309046));
					spawnMonsterOnGroundBelow(mob, new Point(333, 155));
				}
			} else if (슬라임카운트 == 300) {
				broadcastMessage(TheSidPacket.TheSidMessage("After a while another monster will come out! Do not be too nervous!", 3000));
				broadcastMessage(MainPacketCreator.showEffect("Gstar/clearS"));
				newthesid[0] = true;
				startTheSid(2);
			}
			...
			거북이카운트++;
			if (거북이카운트 == 30 || 거북이카운트 == 60 || 거북이카운트 == 90 || 거북이카운트 == 120 || 거북이카운트 == 150 || 거북이카운트 == 180
					|| 거북이카운트 == 210 || 거북이카운트 == 240 || 거북이카운트 == 270 || 거북이카운트 == 300 || 거북이카운트 == 330
					|| 거북이카운트 == 360 || 거북이카운트 == 390 || 거북이카운트 == 420 || 거북이카운트 == 450 || 거북이카운트 == 480) {
				for (int i = 0; i < 30; i++) {
					MapleMonster mob = MapleLifeProvider.getMonster(Randomizer.rand(9309042, 9309043));
					spawnMonsterOnGroundBelow(mob, new Point(333, 155));
				}
			} else if (거북이카운트 == 510) {
				broadcastMessage(TheSidPacket.TheSidMessage("After a while another monster will come out! Do not relax too much!", 3000));
				broadcastMessage(MainPacketCreator.showEffect("Gstar/clearS"));
				newthesid[1] = true;
				startTheSid(3);
			}
			...
			broadcastMessage(MainPacketCreator.showEffect("Gstar/clearS"));
			broadcastMessage(TheSidPacket.TheSidMessage("It is great to kill a Giant Spider!", 3000));
	...
	public void TheSidClear(final int type, MapleCharacter chr) {
		for (final MapleCharacter partymem : chr.getClient().getChannelServer().getPartyMembers(chr.getParty())) {
			...
			partymem.send(UIPacket.getItemTopMsg(2431694, " You have cleared the Tower of Oz and received 200 10th anniversary gift boxes."));
	...
	public void 디멘션인베이드진행(int type, final MapleCharacter chr, MapleMonster monster) {
		if (디멘션인베이드[0] == 1) {
			...
			if (디멘션인베이드[2] == 13 || 디멘션인베이드[2] == 26 || 디멘션인베이드[2] == 39 || 디멘션인베이드[2] == 52) {
				...
				for (final MapleCharacter partymem : chr.getClient().getChannelServer()
						.getPartyMembers(chr.getParty())) {
			...
			if (monster.getId() == 9300622 && monster.getHp() == 0) {
				...
				for (int i = 0; i < 10; i++) {
					...
					spawnMonsterOnGroundBelow(mob1,
							new Point(
									(2794 + (i == 0 ? 10
											: i == 1 ? 50
													: i == 2 ? 100
				...
				if (!엔피시체크(rs.getInt("lifeid"))) {
					{
						...
						if (npc != null) {
							...
							System.err.println("[Error] A null pointer error occurred while creating NPC data.");
						}
					}
				}
			}
		} catch (Exception e) {
			System.err.println("[Error] There was an error fetching NPC from DB.");
			if (!ServerConstants.realese)
				e.printStackTrace();
		}
	}
	...
	public void 베타구출(final MapleCharacter chr, final int a) {
		if (베타구출C[0] == 0) {
			broadcastMessage(
					MainPacketCreator.getNPCTalk(3000131, (byte) 0, "You should be prepared to come here without fear!", "00 00", (byte) 0));
		} else if (베타구출C[0] == 11 && 베타구출C[1] == 11) {
			broadcastMessage(MainPacketCreator.getNPCTalk(3000131, (byte) 0, "He is a reassembly, unfinished, but I wonder if he can stop this guy!!",
					"00 00", (byte) 0));
		}
		{
			EtcTimer tMan = EtcTimer.getInstance();
...

		{
			...
			tMan.schedule(new Runnable() {
				@Override
				public void run() {
					broadcastMessage(TheSidPacket.TopMessage("Powerful monsters appear with dark energy.", 138, 3000));
						...
						for (int i = 0; i < 10; i++) {
							{
								...
								spawnMonsterOnGroundBelow(mob, new Point((short) (369 - Randomizer.nextInt(300)),
										(short) chr.getPosition().getY()));
							}
						}
					} else if (베타구출C[0] == 11 && 베타구출C[1] == 0) {
						베타구출C[1] = 1;
	...
	public void 보스레이드(int i, final long ftime, final long stime) {
		...
		if (보스레이드 == 0)
			broadcastMessage(UIPacket.detailShowInfo("Welcome to the Boss Raid instance.", false));
		보스레이드++;
		{
			EtcTimer tMan = EtcTimer.getInstance();
			tMan.schedule(new Runnable() {
...

		{
			...
			tMan.schedule(new Runnable() {
				...
				public void run() {
					...
					broadcastMessage(UIPacket.detailShowInfo(보스레이드 + "Boss monster Summoned.", false));
	...
	public final void spawnMist(final MapleMist mist, final int duration, boolean poison, boolean fake, boolean rv,
			boolean burningregion, boolean timecapsule, boolean spped, boolean freeze, boolean zero) {
		spawnAndAddRangedMapObject(mist, new DelayedPacketCreation() {
			@Override
			public void sendPackets(MapleClient c) {
				broadcastMessage(MainPacketCreator.spawnMist(mist));
			...
			poisonSchedule = tMan.register(new Runnable() {
				...
				public void run() {
					for (final MapleMapObject mo : getMapObjectsInRect(mist.getBox(),
						...
						if (mist.makeChanceResult()) {
							((MapleMonster) mo).applyStatus(mist.getOwner(),
									new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.SPEED, 1),
											mist.getSourceSkill(), null, false),
									false, duration, false);
							((MapleMonster) mo).applyStatus(mist.getOwner(),
									new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.WDEF, 1),
											mist.getSourceSkill(), null, false),
									false, duration, false);
							((MapleMonster) mo).applyStatus(mist.getOwner(),
									new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.WATK, 1),
											mist.getSourceSkill(), null, false),
									false, duration, false);
						}
					}
			...
			poisonSchedule = tMan.register(new Runnable() {
				...
				public void run() {
					for (final MapleMapObject mo : getMapObjectsInRect(mist.getBox(),
						...
						if (mist.makeChanceResult()) {
							((MapleMonster) mo).applyStatus(mist.getOwner(),
									new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.IMPRINT, 20),
											mist.getSourceSkill(), null, false),
									false, duration, false);
						}
					}
