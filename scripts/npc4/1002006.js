...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
	    var chat = "#b#h #님, 안녕하세요. #k\r\n스킬마스터 담당 #r체프#k라고 합니다.\r\n아래에서 선택해서 눌러주세요.▼\r\n\r\n#k#l"
	    chat += "#e< 모험가 직업군 >#n\r\n";
	    chat += "#L1#히어로#l";
	    chat += "#L2#팔라딘#l";
	    chat += "#L3#다크나이트#l\r\n";
  ...
  dispose();
     ...
     cm.sendOk("#b#h #님,#k 직업은 이게 아닙니다. 다시 선택해주세요.");
     cm.dispose();
 }
} else if (selection == 1) {
...
if (cm.getJob() == 100 || cm.getJob() == 120 || cm.getJob() == 121 || cm.getJob() == 122){
 ...
 if (cm.getJob() == 200 || cm.getJob() == 210 || cm.getJob() == 211 || cm.getJob() == 212){
  ...
  cm.teachSkill(2001002,15,15); // 매직 아머
...
cm.teachSkill(5210015,10,10); // 퀵모션 -- 1차스킬
  ...
  cm.teachSkill(4001014,20,20); // 사이드 스텝
...
cm.teachSkill(5311010,30,30); // 퀵모션 -- 1차스킬
 ...
 cm.teachSkill(22000000,20,20); // 드래곤 소울 -- 1차 스킬 시작
  ...
  cm.teachSkill(22150004,10,10); // 드래곤 스파킹 -- 7차 스킬 끝
 ...
 if (cm.getJob() == 6000 || cm.getJob() == 6100 || cm.getJob() == 6110 || cm.getJob() == 6111 || cm.getJob() == 6112){
  ...
  cm.teachSkill(61120008,10,10); // 파이널 피규레이션
 ...
 if (cm.getJob() == 3200 || cm.getJob() == 3210 || cm.getJob() == 3211 || cm.getJob() == 3212){
  ...
  cm.teachSkill(33000005,20,20); // 오토매틱 슈팅 디바이스  -- 1차 스킬 끝
  cm.teachSkill(33100000,10,10); // 크로스보우 마스터리 -- 2차 스킬 시작 
  cm.teachSkill(33101001,20,20); // 봄 샷
  cm.teachSkill(33101002,10,10); // 재규어 로어 
  cm.teachSkill(33101003,10,10); // 소울 애로우 : 석궁
  ...
  cm.teachSkill(33101012,20,20); // 크로스보우 부스터
  ...
  cm.teachSkill(33111010,10,10); // 와일드 샷
  ...
  cm.teachSkill(33111006,20,20); // 클로우 컷
  cm.teachSkill(33111007,30,30); // 비스트 폼
  cm.teachSkill(33110008,20,20); // 플러리
  cm.teachSkill(33111011,20,20); // 드로우백 -- 3차 스킬 끝
...
cm.sendOk ("스킬마스터가 완료되었습니다.");
 ...
 if (cm.getJob() == 5100 || cm.getJob() == 5110 || cm.getJob() == 5111 || cm.getJob() == 5112){
  ...
  cm.teachSkill(51121008,30,30); // 샤이닝 블라스트
  cm.teachSkill(51121006,30,30); // 소울 레이지
  cm.teachSkill(51120000,15,15); // 컴뱃 마스터리
  cm.teachSkill(51121004,30,30); // 스탠스
  cm.teachSkill(51121005,30,30); // 메이플 용사
cm.sendOk ("스킬마스터가 완료되었습니다.");
  dispose();
