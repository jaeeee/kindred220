...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {	
	    var chat = "안녕하세요~! #p1540211#입니다. 다른 곳으로 안전하고 빠르게 이동하고 싶으신가요? 그렇다면 고객 만족을 최우선으로 생각하는 #b#p1540211##k를 이용해 해보세요. 특별히 무료로! 원하시는 곳까지 친절하게 셔다 드리고 있습니다.\r\n";
	    chat += "#b#L0#"+k+" #e낚시터에선포탈만가능#l\r\n";
	    chat += "#b#L0#"+k+" #e낚시터에선포탈만가능#l\r\n";
	    chat += "#b#L0#"+k+" #e낚시터에선포탈만가능#l\r\n";
	    if(cm.getPlayer().getJob() >= 3300 && cm.getPlayer().getJob() <= 3312) {
	    chat += "#b#L0##e낚시터에선포탈만가능.#l\r\n";
	    }
	    chat += "#b#L0#"+k+" 낚시터에선포탈만가능#l\r\n";
	    cm.sendSimple(chat);
