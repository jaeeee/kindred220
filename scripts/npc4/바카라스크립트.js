...
function action(mode, type, selection) {
	...
	cm.sendGetText("\r\n안녕하세요 #e#d#h0##k#n님. 바카라에 오신것을 환영합니다.\r\n#r#e플레이어(Player)#n#k와 #r#e뱅커(Banker)#k#n에 배팅하셔서 승리하시면 각각 #e#b2배#k#n의 후원포인트를 얻을 수 있고, #e#r비긴다(Tie)#n#k에 배팅하셔서 승리하시면 #e#b5배#k#n의 후원포인트를 얻을 수 있습니다.\r\n#r※주의 : 중간에 종료하시면 아무것도 얻을 수 없습니다.#k\r\n\r\n#b현재 소지중인 후원포인트 : "+cm.getRC()+"#k\r\n\r\n후원포인트를 얼마나 배팅 하시겠습니까?")
    } else if (status == 1) {
	var text = cm.getText();
	if (text > cm.getRC){
	cm.sendOk("소시중인 후원포인트보다 더 많은 포인트를 배팅했습니다.");
	cm.dispose();
	} else if (text < 100){
	cm.sendOk("최소 100포인트 이상을 배팅해야 합니다.");
	cm.dispose();
	} else if (text > 1000000){
	cm.sendOk("최대 1000000포인트 까지 배팅할 수 없습니다.");
	cm.dispose();
	} else {
	var leaf = cm.itemQuantity(4001126);
        var bacara = "#k\r\n"+text+"의 후원포인트를 배팅하셨습니다.\r\n";
	...
	batt += "비겼습니다.\r\n";
	if (sel4 < 3){
	batt += "비겼기 때문에 배팅하신 후원포인트는 다시 돌려드리겠습니다.";
	} else if (sel4 == 3){
	cm.gainRC(text*9);
	batt += ""+text*8+"의 후원포인트를 얻었습니다."
	}
	cm.gainRC(text);
	}
