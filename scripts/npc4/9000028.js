...
var 음치 = Array (2049301,2049401,1662000,1662001,1672000,1662002,1662003,1672022,1662004,1662005,1662006,1662009,1662010,1662011,1662012,1662013,1662014,1662015,1662016,1662019,1662020,1672027,1672040,5064000,5064100,5064300,2470007);
var 음치갯수 = Array (15,15,30,30,30,50,50,50,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,10,10,10,10);
var 몸치 = Array ();
var 몸치갯수 = Array ();
var 박치 = Array ();
var 박치갯수 = Array ();
...
function action(mode, type, selection) {
	...
	if (selection == 0) {
		...
		jessica2 += "#L0##i3010432# 낚시 의자 (3.000.000)\r\n";
		jessica2 += "#L1##i2430996# 미끼 구입 (300.000)";
		...
		if (sel == 1) {
			if (selection == 0) {
				if (cm.getMeso() >= 3000000) {
					...
					cm.sendOk("300만 메소가 있는게 확실한가? 음..다음에 다시 찾아와주게나" );
					cm.dispose();
				}
     			} else if (selection == 1) {
