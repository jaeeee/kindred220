...
function action(mode, type, selection) {
	...
	status--;
    if (status == 0) {
	var change = "안녕하세요…. 혹시 #h #님이 맞으신가요? 위대하신 분을 만나게 되어 영광이에요. 저는  코디네이터 #p9010033#이라고 해요. 특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…! 물론 꼭 지금 바꾸셔야 된다는게 아니라 나중에 천천히 바꾸셔도 되요. #e#r단, 헤어와 성형을 바꾸실 때 부디 검은색으로 바꿔주시고 해주세요….#b#n\r\n\r\n";
	change += "#L1##i5150057# 머리 모양#l ";
	change += "#L3##i5152057# 얼굴#l ";
	//change += "#L6##i1662003# 안드로이드\r\n\r\n";
	...
	if (cm.getPlayer().getGender() == 0) {
		...
		for (var i = 0; i < mhair.length; i++) {
			...
			hairnew.push(mhair[i] + parseInt(cm.getPlayer().getHair2() % 10));
		    }
		}
		cm.askAvatar("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", hairnew);
	    } else if (selection == 2) {
		beauty = 3;
		haircolor = Array();
		var current = parseInt(cm.getPlayer().getHair2() / 10) * 10;
		if (current == 30100) {
		...
		cm.askAvatar("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", haircolor);
	    } else if (selection == 3) {
		beauty = 4;
		facenew = Array();
		for (var i = 0; i < mface.length; i++) {
			...
			facenew.push(mface[i] + cm.getPlayer().getFace2() % 1000 - (cm.getPlayer().getFace2() % 100));
		    }
		}
		cm.askAvatar("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", facenew);
	    } else if (selection == 4) {
		beauty = 5;
		var current = cm.getPlayer().getFace2() % 100 + 20000;
		colors = Array();
		...
		cm.askAvatar("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", colors);
	    }
            else if (selection == 5) {
            if (cm.getPlayer().getGender() == 0) {
                cm.getPlayer().setHair2(31002);
                cm.getPlayer().setFace2(21700);
                cm.getPlayer().setGender(1);
            } else if (cm.getPlayer().getGender() == 1) {
                cm.getPlayer().setHair2(30000);
                cm.getPlayer().setFace2(20100);
		...
		for (var i = 0; i < fhair.length; i++) {
		    hairnew.push(fhair[i] + parseInt(cm.getPlayer().getHair2() % 10));
		}
		cm.askAvatar("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", hairnew);
	    } else if (selection == 2) {
		beauty = 3;
		...
		for (var i = 0; i < fface.length; i++) {
		    facenew.push(fface[i] + cm.getPlayer().getFace2() % 1000 - (cm.getPlayer().getFace2() % 100));
		}
		cm.askAvatar("특별히 #h #님을 위해 무료로 외형을 바꿔드릴게요…. 마음에 드시는 것으로 선택하세요.", facenew);
	    } else if (selection == 4) {
		beauty = 5;
		var current = cm.getPlayer().getFace2() % 100 + 21000;
		colors = Array();
        ...
        if (beauty == 1)
	    setAvatar2(skin[selection]);
        else if (beauty == 2 || beauty == 6)
	    setAvatar2(hairnew[selection]);
        else if (beauty == 3)
	    setAvatar2(haircolor[selection]);
        else if (beauty == 4 || beauty == 7)
	    setAvatar2(facenew[selection]);
        else if (beauty == 5)
	    setAvatar2(colors[selection]);
        if (select == 5) {
            if (cm.getPlayer().getGender() == 0) {
                cm.getPlayer().setHair2(31002);
                cm.getPlayer().setFace2(21700);
...
function setAvatar2(args) {
        if (args < 100) {
            ...
            cm.getPlayer().setFace2(args);
            cm.fakeRelog();
            cm.updateChar();
        } else {
            cm.getPlayer().setHair2(args);
            cm.fakeRelog();
            cm.updateChar();
        }
}