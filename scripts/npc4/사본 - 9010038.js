...
function action(mode, type, selection) {
	...
	var leaf = cm.itemQuantity(4001126);
        var trade = "어떤 주문서를 원하세요?\r\n\r\n#b당신은 현재 단풍잎을 #e#r"+leaf+"#k#n개 가지고 있어요.\r\n주문서는 #r10장#k 단위로밖에 거래할수 없으니 주의해주세요.\r\n";
        ...
        var trade = "원하는 주문서를 골라보세요.\r\n\r\n";
        var scrollArray = getArray(selection);
        for (var i = 0;i < scrollArray.length; i++) {
            trade += "#L"+i+"##i"+scrollArray[i][1]+"# #z"+scrollArray[i][1]+"##l #r10#k장\r\n";
        }
        cm.sendSimple(trade);
    } else if (status == 2) {
        select = selection;
        var scrollArray = getArray(menuSelect);
        cm.sendYesNo("단풍잎 "+scrollArray[select][0]*15+"개를 #b#i"+scrollArray[select][1]+"# #z"+scrollArray[select][1]+"##k#r 10#k장 으로 교환시겠어요? 환불은 안되니 신중하게 결정해주세요.");
    } else if (status == 3) {
        var scrollArray = getArray(menuSelect);
        if (cm.haveItem(4001126, scrollArray[select][0]*15) && cm.canHold(scrollArray[select][1])) {
            cm.gainItem(4001126, -scrollArray[select][0]*15);
            cm.gainItem(scrollArray[select][1], scrollArray[select][2]*10);
