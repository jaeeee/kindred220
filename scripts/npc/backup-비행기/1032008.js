...
function action(mode, type, selection) {
    if (mode <= 0)
        ...
        if (mode == 1)
            ...
            status--;
	if (status == 0) {
		var msg = "오르비스로 가시고 싶으십니까?\r\n\r\n#b#L0#오르비스로 가는 배 탑승#l\r\n";
	...
	if (select == 0) {
		var em = cm.getEventManager(boatevent);
        	if (em.getProperty("entry").equals("true")) {
            		...
            		cm.sendYesNo("아직 배에 탑승할 여유가 있다고 합니다. 오르비스로 가는 배에 탑승하시겠습니까?");
            		...
            		cm.sendOk("이미 배가 출발해서 오르비스 대륙으로 비행중입니다. 매 15분마다 배가 도착하니 조금만 기다려 주시기 바랍니다.");
            		cm.dispose();
        	}
