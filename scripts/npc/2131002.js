...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            var chat = "I'm helping people who are former should not automatically.#b\r\n";
            chat += "\r\n#L0#Let the former.";
            cm.sendSimple(chat);
        } else if (status == 1) {
            if (selection == 0) {
                var jobmain = "Choose a career.#b\r\n";
                jobmain += "\r\n#L0#Let the former as an adventurer.";
                jobmain += "\r\n#L1#Hero will be the former.";
                jobmain += "\r\n#L2#Resistance will be the former.";		
                jobmain += "\r\n#L3#Cygnus to let the former.";
                jobmain += "\r\n#L4#Nova to let the former.";
                cm.sendSimple(jobmain);
			}
        } else if (status == 2) {
            if (selection == 0) {
                var adve = "Choose a career.#b\r\n";
                adve += "\r\n#L0#As will the former champion.";
                adve += "\r\n#L1#Archer to let the former.";
                adve += "\r\n#L2#Let the former as a wizard.";
                adve += "\r\n#L3#Let the former thief with.";
                adve += "\r\n#L4#As will the former pirate.";
                adve += "\r\n#L5#Canon shooters will be the former.";
                adve += "\r\n#L6#Dual Blades will be the former.";
                cm.sendSimple(adve);
            } else if (selection == 1) {
                var hero = "Choose a career.#b\r\n";
                hero += "\r\n#L7#Aran to let the former.";
                hero += "\r\n#L8#Evan as will the former.";
                hero += "\r\n#L9#Phantom to let the former.";
                hero += "\r\n#L10#Let the former Mercedes.";
                hero += "\r\n#L22#Luminous hope the former.";
                hero += "\r\n#L23#Let your ex to eunwol.";
                cm.sendSimple(hero);
		    ...
		    var warrior = "전사에 대해 알고 싶은가?#b\r\n";
                    if (cm.getPlayer().getLevel() >= 10 && cm.getJob() == 0) {
						...
						cm.sendSimple(warrior);
		   				...
		   				warrior += "\r\nThis condition can not be met former.";
						cm.sendOk(warrior);
					}
		} else if (selection == 1) {
		...
		if (selection == 15) {
		    ...
		    var kaiser = "카이저가 되고 싶은거야?#b\r\n";
                    if (cm.getPlayer().getLevel() >= 10 && cm.getJob() == 0) {
                    	...
                    	kaiser += "\r\n#L187#Let the former KAISER (3rd).";
						cm.sendSimple(kaiser);
					}
                    else if (cm.getPlayer().getLevel() >= 100 && cm.getJob() == 6111) {
                    	kaiser += "\r\n#L188#Let the former KAISER (4 cars).";
						cm.sendSimple(kaiser);
		    ...
		    var angelicbuster = "Angelic Buster'll want to be?#b\r\n";
                    if (cm.getPlayer().getLevel() >= 10 && cm.getJob() == 0) {
                    	angelicbuster += "\r\n#L82#As will the former Angelic Buster (1st).";
						cm.sendSimple(angelicbuster);
					}
                    else if (cm.getPlayer().getLevel() >= 30 && cm.getJob() == 6500) {
                    	angelicbuster += "\r\n#L83#As will the former Angelic Buster (2nd).";
						cm.sendSimple(angelicbuster);
					}
                    else if (cm.getPlayer().getLevel() >= 60 && cm.getJob() % 100 == 10) {
                    	angelicbuster += "\r\n#L84#As will the former Angelic Buster (3rd).";
						cm.sendSimple(angelicbuster);
					}
                    else if (cm.getPlayer().getLevel() >= 100 && cm.getJob() % 100 == 11) {
                    	angelicbuster += "\r\n#L84#As will the former Angelic Buster (4th).";
						cm.sendSimple(angelicbuster);
