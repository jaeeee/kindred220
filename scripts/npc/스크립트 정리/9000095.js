...
function action(mode, type, selection) {
    ...
    if (status == 0) {
var mainc = "#r#h0##k님 저는 보조무기지급을 담당한 예븐 할로원 마녀 입니다.\r\n\r\n";
mainc += "#b#L10##i1353100##l";
...
cm.addEquip(-10, 1353101, 0, 0, 0, 0, 0, 0);
cm.fakeRelog();
cm.updateChar();
cm.dispose();
}else if (cm.getPlayer().getLevel() >= 10 && cm.getPlayer().getLevel() < 100) {
cm.addEquip(-10, 1353102, 0, 0, 0, 0, 0, 0);
cm.fakeRelog();
cm.updateChar();
cm.dispose();
}else if (cm.getPlayer().getLevel() >= 10 && cm.getPlayer().getLevel() < 250) {
cm.addEquip(-10, 1353103, 0, 0, 0, 0, 0, 0); //은월
cm.fakeRelog();
cm.updateChar();
cm.dispose();
}else{
