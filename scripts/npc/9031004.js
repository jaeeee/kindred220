...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var menu = "예쁘고 멋진 장신구들을 제작하는~ 장신구 제작에 관심이 있나?\r\n";
        menu += "#L0##b#e장신구 제작#n에 대한 설명을 듣는다.#l\r\n";
        if (cm.getProfession(2) != MapleProfessionType.ACC.getValue()) {
            menu += "#L1##e장신구 제작#n을 배운다.#l\r\n";
        }
        if (cm.getProfession(2) == MapleProfessionType.ACC.getValue()) {
            menu += "#L2##e장신구 제작#n을 초기화 한다.#l\r\n";
        }
        if (cm.getProfession(2) == MapleProfessionType.ACC.getValue() && cm.getPlayer().getProfession().getSecondProfessionExp() == GameConstants.getProfessionExpNeededForLevel(cm.getPlayer().getProfession().getSecondProfessionLevel())) {
            menu += "#L3##e장신구 제작#n의 레벨을 한 단계 올린다.#l\r\n";
        }
        cm.sendSimple(menu);
    } else if (status == 1) {
        select = selection;
        if (selection == 0) {
            cm.sendOk("장신구 제작은 제련된 보석과 광물을 이용하여 멋진 장신구들을 만드는 매력적인 기술이라네. 반지면 반지, 어깨, 벨트 등 못만드는게 없다네~!");
            cm.dispose();
            return;
        } else if (selection == 1) {
            if (cm.getProfession(1) == MapleProfessionType.MINING.getValue() && cm.getProfession(2) == MapleProfessionType.NONE.getValue()) {
                ...
                cm.sendOk("이미 자네는 장비 제작을 배운것 같은데? 그렇다면 장신구 제작을 배울 수 없다네.");
                cm.dispose();
                return;
            } else {
                cm.sendOk("장신구 제작은 채광을 배운 후에 가르쳐 줄 수 있다네. 채광은 오른쪽의 #b노붐#k에게서 배울 수 있지.");
                cm.dispose();
                return;
            }
        } else if (selection == 2) {
            ...
            if (cm.getProfession(2) == MapleProfessionType.ACC.getValue()) {
                cm.sendYesNo("#b장신구 제작#k의 레벨을 한단계 올려주겠네. 수수료는 #b"+(cm.getPlayer().getProfession().getSecondProfessionLevel()*100000)+"메소#k 라네. 정말 배우고 싶나?");
            }
        }
    } else if (status == 2) {
        if (select == 1) {
            if (cm.getPlayer().getMeso() >= 5000) {
                ...
                cm.sendOk("메소가 부족한데? 미안하지만 돈이 없으면 장신구 제작을 가르쳐 줄 수 없다구.");
                cm.dispose();
                return;
            }
        } else if (select == 2) {
            ...
            if (cm.getPlayer().getMeso() >= (cm.getPlayer().getProfession().getSecondProfessionLevel()*100000)) {
                ...
                cm.sendOk("장신구 제작의 기술이 한단계 올랐다네.");
                cm.dispose();
            } else {
