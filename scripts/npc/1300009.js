...
function action(mode, type, selection) {
 status++;
 if (status == 0) {
  cm.sendSimple("메소로 보스를 소환할 수 있습니다.\r\n#L1##r" + cost + "#b 메소로 머쉬맘 소환#l\r\n#L3##r" + cost + "#b 메소로 블루머쉬맘 소환#l\r\n#L4##r" + cost + "#b 메소로 좀비머쉬맘 소환#l\r\n#L2#헤네시스로 가기#l#k\r\n");
 } else if(status == 1) {
  if(selection == 1) {
 ...
 WorldBroadcasting.broadcast(MainPacketCreator.serverNotice(6, cm.getPlayer().getName()+"님께서 머쉬맘을 소환하였습니다.")); // 게임 채팅서버알림 메세지 
 ...
 WorldBroadcasting.broadcast(MainPacketCreator.serverNotice(6, cm.getPlayer().getName()+"님께서 블루머쉬맘을 소환하였습니다.")); // 게임 채팅서버알림 메세지 
 cm.gainMeso(-cost); // 메소 차감시키기
