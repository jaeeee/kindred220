...
function action(mode, type, selection) {
	...
	var ADVR = "#r모험가#k를 선택 하셨군요, 모험가는 이전부터 메이플 월드를 모험하던 #b이방인#k에 지나지 않았지만, 메이플 월드를 모험하다 검은 마법사의 위협을 받아 그에 반격하는 유망주가 되게 됩니다.#b\r\n";
        ...
        //ADVR += "#L3#단검을 다루는 모험가, 도적으로 전직하겠습니다.#l\r\n";
	...
	var Zero = "당신이 여신의 후계자이며 새로운 초월자인 그 말로만듣던 신의아이인가요?\r\n";
        Zero += "#L100##b제로(신의아이)로 전직하겠습니다.#l\r\n";
	cm.sendSimple(Zero);
        } else if (selection == 6) {
	...
	cm.sendSimple(Warreor); 
        	...
        	Warreor += "#L3#스피어맨(창)으로 전직하겠습니다.#l\r\n";
	cm.sendSimple(Warreor); 
	...
	cm.sendSimple(Eunwol1);
	    ...
	    cm.dispose();
            ...
            cm.getPlayer().setLevel(60);
            cm.warp(101072000,0);cm.changeJob(131);
	    cm.getPlayer().gainAp(200);
            cm.teachSkill(1301013,0,0); // 비홀더
            cm.teachSkill(1321007,0,0); // 비홀더
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("지금 전사(창 계열) 2차 전직은 오류 수정 중 이라 이용이 불가능해. 참고하고 채널 변경시 3차 전직과 레벨 60으로 설정이 완료된다.");
	    cm.dispose();
        } else if (selection == 4) {
	 ...
	 cm.teachSkill(1320008,0,20); // 비홀더스 힐링
	  ...
	  cm.teachSkill(3220012,0,15); // 엘리트 퍼펫
	    ...
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("채널은 변경하시면 레벨 70으로 설정이완료됩니다. 현재 도적(단검계열)은 오류가발견되어 임시로 2차 전직을 이용하실 수 없습니다.");
	    cm.dispose();
  	...
  	cm.teachSkill(60010217,1,1); // 트루 석세서
	    ...
	    cm.playerMessage(5, "제로 전직을 완료하였습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
