...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		for (var x = 0; x < invs.length; x++) {
			var inv = cm.getInventory(operation);
			for (var i = 0; i <= inv.getSlotLimit(); i++) {
				...
				//
				if (!cm.getClient().getPlayer().getInventory(type)) {
					continue;
				}
				ok = true;
				selStr += "#L" + (invs[operation] * 1000 + i) + "##v" + itemid + "##t" + itemid + "##l\r\n";
			}
		}
		if (!ok) {
		...
		cm.sendSimple(selStr + "#k");
			...
			var conn = cm.getClient().getChannelServer().getPlayerStorage().getCharacterByName(text);
			InventoryManipulator.removeFromSlot(cm.getC(), type, selection%1000, 1, true);
