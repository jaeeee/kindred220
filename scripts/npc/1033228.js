...
var mobcode = Array(1,2,8850011,9300461,9300708,8870000,8880010,1,2,8840000,8860000,8820200,9303101,8930000,8930001);
function start() {
    status = -1;
    action(1, 0, 0);
}
	...
	if (status == 0) {
		if(cm.isLeader()) {
			if(cm.getMonsterCount(cm.getPlayer().getMapId()) == 0) {
				cm.sendYesNo("#r[Kindred Boss Manager]#k\r\nAre you ready to spawn the boss?");
			} else {
		...
		if (cm.getPlayer().getMapId() == 271040100) {
			...
			//WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[Human online] : " + cm.getPlayer().getName() + "님이 시그너스를 소환하였습니다."));
		} else if(cm.getPlayer().getMapId() == 401060200) {
			cm.spawnMob(mobcode[6],cm.getPlayer().getPosition().getX(),cm.getPlayer().getPosition().getY());
			//WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[Human online] : " + cm.getPlayer().getName() + "님이 매그너스를 소환하였습니다."));
		} else if(cm.getPlayer().getMapId() == mapid[9]) {
			cm.spawnMob(mobcode[9],cm.getPlayer().getPosition().getX(),cm.getPlayer().getPosition().getY());
			//WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[Human online] : " + cm.getPlayer().getName() + "님이 반 레온을 소환하였습니다."));
		} else if(cm.getPlayer().getMapId() == mapid[10]) {
			cm.spawnMob(mobcode[10],cm.getPlayer().getPosition().getX(),cm.getPlayer().getPosition().getY());
			//WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[Human online] : " + cm.getPlayer().getName() + "님이 아카이럼을 소환하였습니다."));
			...
			//WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[Human online] : " + cm.getPlayer().getName() + "님이 벨룸을 소환하였습니다."));
		} else if(cm.getPlayer().getMapId() == 262031300) {
			cm.spawnMob(8870100,cm.getPlayer().getPosition().getX(),cm.getPlayer().getPosition().getY());
			//WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[Human online] : " + cm.getPlayer().getName() + "님이 힐라을 소환하였습니다."));
