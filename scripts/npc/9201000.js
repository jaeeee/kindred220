...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
	if (status == 0) {
		var huwon = "   #e안녕하세요. #e#b#h0#님!!!#n#k\r\n   저희 #e#r휴먼온라인#n#k파티퀘스트 에 오신걸 환영합니다!\r\n";
		if (cm.getPlayer().getMapId() == 350060908) {
	    	...
	    	huwon += "   #e#b#i4033914# #z4033914##n#k이 있으면 #e#b파티퀘스트#n#k를 #e#b이용#n#k하실 수\r\n   있습니다. 어때여? #e#b한 번#n#k #e#r이용#n#k해 보실래요?\r\n\r\n";
		huwon += "#L350060908##e#b파티퀘스트 이동하기\r\n";
		}
		cm.sendSimple(huwon);
	} else if (status == 1) {
		if (selection == 350060908) {
			...
			if (cm.getPlayer().getLevel() < 50) {
				WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[메리스토리] : " + cm.getPlayer().getName() + "님이 파티퀘스트의 몬스터를 소환하였습니다."));
				...
				WorldBroadcasting.broadcastMessage(MainPacketCreator.serverNotice(2, "[메리스토리] : " + cm.getPlayer().getName() + "님이 후원사냥터의 몬스터를 소환하였습니다."));
			...
			if (cm.getPlayerCount(selection) > 0) {
				cm.sendOk("이미 다른 유저가 #e#r파티퀘스트#n#k를 #e#b이용 중#n#k 입니다.\r\n#e#r다른 채널#n#k에서 #e이용#n#k해 주세요.");
				cm.dispose();
			} else if (cm.haveItem(4033914, 1)) {
				cm.warp(selection);
				cm.dispose();
			} else {
				cm.sendOk("#e#b#i4033914# #z4033914##n#k이 있는지 #e#r확인#n#k해 주세요.");
				cm.dispose();
			}		
		}
	}
