...
function action(mode, type, selection) {
	if (mode == -1) {
		cm.sendOk("혹시라도 결정이 서신다면 저에게로 오세요~ 전직의 문은 열려있답니다~ 전직을 재때 하지 못하시면 사냥을 아무리 해도 얻는 것이 없는건 알고 계시죠?");
		cm.dispose();
	} else {
	...
	status--;
    if (status == 0) {
	var intro = "안녕하세요 #b#h0##k님.\r\n#b원하시는 직업#k을 선택해주세요.\r\n";
               // intro += "#L0##b모험가(#r듀얼블레이드 + 캐논슈터 포함#b)로 전직하고 싶어요#l\r\n";
	//intro += "#L1#5대 영웅(#r아란/에반/메르/팬텀/루미너스#b)으로 전직하고 싶어요#l\r\n";
	//intro += "#L2#시그너스 기사단(#r미하일 포함#b)으로 전직하고 싶어요#l\r\n";
	intro += "#L3#전직 하겠습니다.";
	//intro += "#L4#노바(#r카이저 / 엔젤릭버스터#b)로 전직하고 싶어요#l\r\n";
	cm.sendSimple(intro);
     } else if (status == 1) {
        if (selection == 0) {
	var ADVR = "#r모험가#k를 선택 하셨군요, 모험가는 이전부터 메이플 월드를 모험하던 #b이방인#k에 지나지 않았지만, 메이플 월드를 모험하다 검은 마법사의 위협을 받아 그에 반격하는 유망주가 되게 됩니다.#b\r\n";
        ...
        ADVR += "#L5##r#e[스페셜]#n#b 이도류의 종결자, 듀얼블레이드로 전직하겠습니다.#l\r\n";
        ADVR += "#L6##r#e[스페셜]#n#b 캐논의 정석, 캐논슈터로 전직하겠습니다.#l\r\n";
	cm.sendSimple(ADVR);
        } else if (selection == 1) {
	...
	var CY = "#r시그너스 기사단#k를 선택 하셨군요, 시그너스 기사단은 여제 시그너스와 신수를 위해 봉사하는 기사단원, 기사단장 등으로 이루어져 있습니다.#b\r\n";
        ...
        CY += "#L17##r#e[스페셜]#b#n 기사단장 미하일에 관심이 있습니다.#l\r\n";
	cm.sendSimple(CY);
        } else if (selection == 3) {
	var Regestangs = "#r자동 전직이 안되는 점은 죄송합니다.\r\n#k 최대한 빠르게 고치도록 하겠습니다.\r\n";
        Regestangs += "#L18#배틀메이지#l\r\n";
        Regestangs += "#L19#와일드헌터#l\r\n";
        //Regestangs += "#L20#메카닉#l\r\n";
        Regestangs += "#L21#데몬슬레이어#l\r\n";
        Regestangs += "#L37#데몬어벤져#l\r\n";
        Regestangs += "#L11#루미너스#l\r\n";
        Regestangs += "#L38#제논#l\r\n";
        Regestangs += "#L8#에반#l\r\n";
	...
	cm.sendSimple(Prirat); 
              } else if (cm.getPlayer().getLevel() >= 120 && cm.getPlayer().getJob() == 511) {
	...
	var Dual = "#r#e[스페셜 모험가]#b#n 듀얼블레이드#k를 선택 하셨군요, 듀얼블레이드는 말그대로 #b이도류의 종결자#k이며 어느 도적보다 재빠른 공격/이동 속도를 자랑합니다, 현 다크로드의 전대 다크로드를 따르던 설희의 제자들이 바로 듀얼블레이드 입니다.#b\r\n";
              if (cm.getPlayer().getLevel() >= 1 && cm.getPlayer().getJob() == 0) {
        	Dual += "#L34#듀얼블레이드(이도류)으로 진출하려고 합니다.#l\r\n";
	cm.sendSimple(Dual); 
              } else if (cm.getPlayer().getLevel() >= 30 && cm.getPlayer().getJob() == 431) {
        	Dual += "#L35#2차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Dual); 
              } else if (cm.getPlayer().getLevel() >= 55 && cm.getPlayer().getJob() == 432) {
        	Dual += "#L36#3차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Dual); 
              } else if (cm.getPlayer().getLevel() >= 60 && cm.getPlayer().getJob() == 433) {
        	Dual += "#L37#3+차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Dual); 
              } else if (cm.getPlayer().getLevel() >= 100 && cm.getPlayer().getJob() == 434) {
	...
	var Canon = "#r#e[스페셜 모험가]#b#n 캐논슈터#k를 선택 하셨군요, 캐논슈터는 #b빠른 공격 속도#k를 자랑하는 해적의 스페셜 모험가입니다.#b\r\n";
              if (cm.getPlayer().getJob() == 0) {
	...
	cm.sendSimple(A);
	      } else if (cm.getPlayer().getLevel() >= 120 && cm.getPlayer().getJob() == 1111) {
	...
	var f = "#r#e[스페셜 기사단]#n#b 미하일#k을 선택 하셨군요, 미하일은 시그너스 기사단의 기사단장이며, #b빛의 속성#k을 사용하는 기사단장 입니다.#b\r\n";
	...
	var Demon = "#r#e[스페셜 레지스탕스]#n#b 데몬슬레이어#k를 선택 하셨군요, 데몬슬레이어는 한때 검은 마법사의 군단장이였으며, 검은 마법사가 배신한 이후, 레지스탕스들과 같이 블랙윙에 맞서게 됩니다. 데몬슬레이어는 #b포스#k를 사용하는 스페셜 레지스탕스 입니다.#b\r\n";
              if (cm.getPlayer().getJob() == 3000) {
	...
	var Demon2 = "#r#e[유니크 레지스탕스]#n#b 데몬어벤져#k를 선택 하셨군요, 데몬어벤져는 요번 새로 추가가 된 직업이며 #d데몬슬레이어#k가 개편된 직업입니다.#b\r\n";
              if (cm.getPlayer().getJob() == 3000) {
         	Demon2 += "#L221#데몬어벤져로 입문하려고 합니다.#l\r\n";
	cm.sendSimple(Demon2);
	      } else if (cm.getPlayer().getLevel() >= 30 && cm.getPlayer().getJob() == 3101) {
         	Demon2 += "#L222#2차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Demon2);
	      } else if (cm.getPlayer().getLevel() >= 60 && cm.getPlayer().getJob() == 3120) {
         	Demon2 += "#L223#3차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Demon2);
	      } else if (cm.getPlayer().getLevel() >= 100 && cm.getPlayer().getJob() == 3121) {
         	Demon2 += "#L224#대망의 4차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Demon2);
	 } else {
		Demon2 += "\r\n당신은 이미 전직을 마치셨거나, 전직할수 있는 상태가 아닙니다.";
			cm.sendNext(Demon2);
	...
	var Xenon = "#r#e[레전드 레지스탕스]#n#b 제논#k을 선택 하셨군요, 제논은 요번 새로 추가가 된 직업이며 아주 인기가 많은 직업이고 #d레지스탕스#k에서 비밀요원으로 남은 유일한 직업입니다.#b\r\n";
              if (cm.getPlayer().getJob() == 3000) {
         	Xenon += "#L225#제논 으로 입문하려고 합니다.#l\r\n";
	cm.sendSimple(Xenon);
	      } else if (cm.getPlayer().getLevel() >= 30 && cm.getPlayer().getJob() == 3600) {
         	Xenon += "#L226#2차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Xenon);
	      } else if (cm.getPlayer().getLevel() >= 60 && cm.getPlayer().getJob() == 3610) {
         	Xenon += "#L227#3차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Xenon);
	      } else if (cm.getPlayer().getLevel() >= 100 && cm.getPlayer().getJob() == 3611) {
         	Xenon += "#L228#대망의 4차 전직을 하고 싶습니다.#l\r\n";
	cm.sendSimple(Xenon);
	 } else {
		Xenon += "\r\n당신은 이미 전직을 마치셨거나, 전직할수 있는 상태가 아닙니다.";
			cm.sendNext(Xenon);
	...
	cm.sendSimple(NOVA2);
        ...
        if (selection == 0) {
            ...
            cm.warp(1000000,0);cm.changeJob(100);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(110);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 2) {
            cm.warp(1000000,0);cm.changeJob(120);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 3) {
            cm.warp(1000000,0);cm.changeJob(130);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 4) {
            cm.warp(1000000,0);cm.changeJob(111);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(121);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(131);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
 	    ...
 	    cm.teachSkill(1121010,0,30); // 인레이지
  	    ...
  	    cm.teachSkill(1120013,0,30); // 어드밴스드 파이널 어택 -- 4차끝
            cm.warp(1000000,0);cm.changeJob(112);
	  ...
	  cm.teachSkill(1221011,0,30); // 생츄어리
            cm.warp(1000000,0);cm.changeJob(122);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	 ...
	 cm.teachSkill(1320008,0,20); // 비홀더스 힐링
 	 ...
 	 cm.teachSkill(1321012,0,30); // 다크 임페일
            cm.warp(1000000,0);cm.changeJob(132);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(200);
	    cm.gainItem(1142107, 1);
		cm.teachSkill(190,1,1);
	    cm.playerMessage(5, "1차 전직이 완료 되었습니다, 필요한 무기는 기본 상점에서 1000메소에 구매가 가능합니다.")
	    cm.playerMessage(5, "<초보 모험가> 칭호를 획득 하셨습니다.")
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(210);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 12) {
            cm.warp(1000000,0);cm.changeJob(220);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 13) {
            cm.warp(1000000,0);cm.changeJob(230);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 14) {
            cm.warp(1000000,0);cm.changeJob(211);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(221);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(231);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(2120010,0,30); // 아케인 에임
            cm.warp(1000000,0);cm.changeJob(212);
	    cm.gainItem(1142110, 1);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(2220010,0,30); // 아케인 에임
            cm.warp(1000000,0);cm.changeJob(222);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	  ...
	  cm.teachSkill(2320011,0,30); // 아케인 에임
            cm.warp(1000000,0);cm.changeJob(232);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();	
            ...
            cm.warp(1000000,0);cm.changeJob(300);
	    cm.gainItem(1142107, 1);
		cm.teachSkill(190,1,1);
	    cm.playerMessage(5, "1차 전직이 완료 되었습니다, 필요한 무기는 기본 상점에서 1000메소에 구매가 가능합니다.")
	    cm.playerMessage(5, "<초보 모험가> 칭호를 획득 하셨습니다.")
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(310);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 22) {
            cm.warp(1000000,0);cm.changeJob(320);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 23) {
            cm.warp(1000000,0);cm.changeJob(311);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(321);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(3120012,0,15); // 엘리트 퍼펫
            cm.warp(1000000,0);cm.changeJob(312);
	    cm.gainItem(1142110, 1);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(3220012,0,15); // 엘리트 퍼펫
            cm.warp(1000000,0);cm.changeJob(322);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(410);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 29) {
            cm.warp(1000000,0);cm.changeJob(420);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 30) {
            cm.warp(1000000,0);cm.changeJob(411);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(421);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(4121016,0,30); // 써든레이드
            cm.warp(1000000,0);cm.changeJob(412);
	    cm.gainItem(1142110, 1);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(4221013,0,20); // 섀도어 인스팅트
            cm.warp(1000000,0);cm.changeJob(422);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(430);
	  ...
	  cm.teachSkill(4341008,5,5); 
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(431);
	    ...
	    cm.dispose();        
            ...
            cm.warp(1000000,0);cm.changeJob(432);
  	    cm.teachSkill(4321006,0,20); // 플라잉 어썰터 -- 4차스킬 끝
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 37) {
            cm.warp(1000000,0);cm.changeJob(433);
	    cm.gainItem(1142109, 1);
	  ...
	  cm.teachSkill(4341009,0,30); // 팬텀 블로우
            cm.warp(1000000,0);cm.changeJob(434);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.resetStats(4,25,4,4);
            cm.warp(1000000,0);cm.changeJob(500);
	    cm.gainItem(1142107, 1);
		cm.teachSkill(190,1,1);
	    cm.playerMessage(5, "1차 전직이 완료 되었습니다, 필요한 무기는 기본 상점에서 1000메소에 구매가 가능합니다.")
	    cm.playerMessage(5, "<초보 모험가> 칭호를 획득 하셨습니다.")
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(510);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 41) {
            cm.warp(1000000,0);cm.changeJob(520);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 42) {
            cm.warp(1000000,0);cm.changeJob(511);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(521);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(5121016,0,30); // 에너지 블라스트
            cm.warp(1000000,0);cm.changeJob(512);
	    cm.gainItem(1142110, 1);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.")
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(5220020,0,30); // 캡틴 디그니티
            cm.warp(1000000,0);cm.changeJob(522);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.resetStats(4,25,4,4);
            cm.warp(1000000,0);cm.changeJob(501);
	    ...
	    cm.teachSkill(112,2,2); // 수납의 달인
            cm.warp(1000000,0);cm.changeJob(530);
	    cm.gainItem(1142108, 1);
	    cm.playerMessage(5, "<주니어 모험가> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 48) {
            cm.warp(1000000,0);cm.changeJob(531);
	    cm.gainItem(1142109, 1);
	    cm.playerMessage(5, "<베테랑 모험가> 칭호를 획득 하셨습니다.");
	    cm.getPlayer().gainAp(5);
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	  ...
	  cm.teachSkill(5320007,0,5); // 더블 럭키 다이스
            cm.warp(1000000,0);cm.changeJob(532);
	    cm.gainItem(1142110, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<마스터 모험가> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.resetStats(35,4,4,4);
            cm.warp(1000000,0);cm.changeJob(2100);
		...
		cm.teachSkill(20000194,1,1); //되찾은 기억
            ...
            cm.warp(1000000,0);cm.changeJob(2110);
	    cm.gainItem(1142130, 1);
	    cm.playerMessage(5, "<기억속의 아란> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 52) {
            cm.warp(1000000,0);cm.changeJob(2111);
	  ...
	  cm.teachSkill(21120005,0,30); // 파이널 블로우
	  cm.teachSkill(21120006,0,30); // 콤보 템페스트
	  cm.teachSkill(21120007,0,30); // 콤보 배리어
            cm.warp(1000000,0);cm.changeJob(2112);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(4,4,25,4);
            cm.warp(1000000,0);cm.changeJob(2200);
	    cm.gainSp(2);
            if (cm.getPlayer().getGender() == 0) {
                cm.getPlayer().setFace(20000);
                cm.getPlayer().setHair(30027);
		...
		cm.teachSkill(20010194,1,1); //계승된 의지
	   	...
	   	cm.playerMessage(5, "<착한 아이> 칭호를 획득 하셨습니다.");
            ...
            cm.warp(1000000,0);cm.changeJob(2210);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(2211);
	    cm.gainItem(1142154, 1);
	    cm.playerMessage(5, "<커닝시티 명예시민> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 57) {
            cm.warp(1000000,0);cm.changeJob(2212);
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 58) {
            cm.warp(1000000,0);cm.changeJob(2213);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(2214);
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 60) {
            cm.warp(1000000,0);cm.changeJob(2215);
	    cm.gainItem(1142156, 1);
	    cm.playerMessage(5, "<드래곤 마스터> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 61) {
            cm.warp(1000000,0);cm.changeJob(2216);
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 62) {
            cm.warp(1000000,0);cm.changeJob(2217);
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(22171003,0,30); // 플레임 휠
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(2218);
	    cm.getPlayer().gainAp(5);
	  ...
	  cm.teachSkill(22181004,0,20); // 오닉스의 의지
	    ...
	    cm.dispose();
            ...
            cm.resetStats(4,25,4,4);
            cm.warp(1000000,0);cm.changeJob(2300);
	    cm.gainSp(2);
            if (cm.getPlayer().getGender() == 0) {
               cm.resetEquip();
	  	...
	  	cm.teachSkill(20020190,1,1); // 연합의 의지
            ...
            cm.warp(1000000,0);cm.changeJob(2310);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(2311);
	  ...
	  cm.teachSkill(23121005,0,30); // 메이플 용사
            cm.warp(1000000,0);cm.changeJob(2312);
	    ...
	    cm.dispose();	
            ...
            cm.resetStats(4,4,4,25);
            cm.warp(1000000,0);cm.changeJob(2400);
	    cm.gainSp(2);
            if (cm.getPlayer().getGender() == 0) {
	  	...
	  	cm.teachSkill(20031208,1,1); // 스킬 매니지먼트
 	  	cm.teachSkill(20031209,1,1); // 저지먼트 -- 0차스킬 끝
            ...
            cm.warp(1000000,0);cm.changeJob(2410);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(2411);
	  ...
	  cm.teachSkill(20031210,1,1); // 저지먼트 강화
            cm.warp(1000000,0);cm.changeJob(2412);
		...
		cm.openNpc(1032216);
            ...
            cm.warp(1000000,0);cm.changeJob(2710);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(2711);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(2712);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(35,4,4,4);
	    cm.gainSp(2);
            cm.warp(1000000,0);cm.changeJob(1100);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(4,4,25,4);
            cm.warp(1000000,0);cm.changeJob(1200);
	    cm.gainSp(2);
	    cm.gainItem(1142066, 1);
	    cm.teachSkill(10000190,1,1); // 연합의 의지
	    cm.playerMessage(5, "<수련기사> 칭호를 획득 하셨습니다.");
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1210);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1211);
	    cm.gainItem(1142068, 1);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1212);
	    cm.gainItem(1142069, 1);
	    cm.teachSkill(10001005,1,1); // 영웅의 메아리
	    cm.playerMessage(5, "<기사단장> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.resetStats(4,25,4,4);
            cm.warp(1000000,0);cm.changeJob(1300);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1310);
	    cm.gainItem(1142067, 1);
	    cm.playerMessage(5, "<정식기사> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 86) {
            cm.warp(1000000,0);cm.changeJob(1311);
	    cm.gainItem(1142068, 1);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1312);
	    cm.gainItem(1142069, 1);
	    cm.teachSkill(10001005,1,1); // 영웅의 메아리
	    cm.playerMessage(5, "<기사단장> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.resetStats(4,4,4,25);
            cm.warp(1000000,0);cm.changeJob(1400);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1410);
	    cm.gainItem(1142067, 1);
	    cm.playerMessage(5, "<정식기사> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 90) {
            cm.warp(1000000,0);cm.changeJob(1411);
	    cm.gainItem(1142068, 1);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1412);
	    cm.gainItem(1142069, 1);
	    cm.teachSkill(10001005,1,1); // 영웅의 메아리
	    cm.playerMessage(5, "<기사단장> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1500);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1510);
	    cm.gainItem(1142067, 1);
	    cm.playerMessage(5, "<정식기사> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 94) {
            cm.warp(1000000,0);cm.changeJob(1511);
	    cm.gainItem(1142068, 1);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(1512);
	    cm.gainItem(1142069, 1);
	    cm.teachSkill(10001005,1,1); // 영웅의 메아리
	    cm.playerMessage(5, "<기사단장> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
             ...
             if (cm.getPlayer().getGender() == 0) {
            	cm.resetStats(35,4,4,4);
	    	cm.gainSp(2);	
		cm.warp(1000000,0);cm.changeJob(5100);
  		...
  		cm.teachSkill(50001214,1,1); // 빛의수호 -- 0차스킬
            ...
            cm.warp(1000000,0);cm.changeJob(5110);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(5111);
  	...
  	cm.teachSkill(51121005,0,30); // 메이플 용사
            cm.warp(1000000,0);cm.changeJob(5112);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(4,4,25,4);
            cm.warp(1000000,0);cm.changeJob(3200);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3210);
	    cm.gainItem(1142243, 1);
	    cm.playerMessage(5, "<특별수업 중급생> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 102) {
            cm.warp(1000000,0);cm.changeJob(3211);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3212);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(4,25,4,4);
	    cm.gainSp(2);
            cm.warp(1000000,0);cm.changeJob(3300);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3310);
	    cm.gainItem(1142243, 1);
	    cm.playerMessage(5, "<특별수업 중급생> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 106) {
            cm.warp(1000000,0);cm.changeJob(3311);
	    cm.gainItem(1142244, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<특별수업 상급생> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3312);
	    cm.gainItem(1142245, 1);
	    cm.getPlayer().gainAp(5);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(4,25,4,4);
            cm.warp(1000000,0);cm.changeJob(3500);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3510);
	    cm.gainItem(1142243, 1);
	    cm.playerMessage(5, "<특별수업 중급생> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 110) {
            cm.warp(1000000,0);cm.changeJob(3511);
	    cm.gainItem(1142244, 1);
	    cm.getPlayer().gainAp(5);
	    cm.playerMessage(5, "<특별수업 상급생> 칭호를 획득 하셨습니다.");
	    cm.sendNext("당신에게 약간의 #rAP와 SP#k를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3512);
	    cm.gainItem(1142245, 1);
	    cm.getPlayer().gainAp(5);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(35,4,4,4);
            cm.warp(1000000,0);cm.changeJob(3100);
	  	...
	  	cm.teachSkill(30010112,1,1); // 데몬스 퓨리
            ...
            cm.warp(1000000,0);cm.changeJob(3110);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3111);
		...
		cm.teachSkill(31121007,0,15); // 인피니티 포스
  		...
  		cm.teachSkill(31120009,0,30); // 하드 스킨
            cm.warp(1000000,0);cm.changeJob(3112);
	    ...
	    cm.dispose();
            ...
            cm.resetStats(35,4,4,4);
            cm.warp(1000000,0);cm.changeJob(3101);
                //cm.getPlayer().setKeyValue("AutoJob", "3110");
            if (cm.getPlayer().getGender() == 0) {
                cm.resetEquip();
	    	cm.gainSp(2);
	  	...
	  	cm.teachSkill(30010112,1,1); // 데몬스 퓨리
            ...
            cm.warp(1000000,0);cm.changeJob(3120);
	    cm.gainItem(1142342, 1);
	    cm.addEquip(-10, 1099002, 0, 30, 15, 7, 150, 50); //포스 실드
	    cm.fakeRelog();
            cm.updateChar();
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(3121);
	    cm.gainItem(1142343, 1);
	    cm.getPlayer().gainAp(5);
	    ...
	    cm.dispose();
  		...
  		cm.teachSkill(31221000,0,30); // 데몬 익스플로젼
  		cm.teachSkill(31221001,0,30); // 데몬 임팩트
  		cm.teachSkill(31221002,0,20); // 데빌 크라이
  		cm.teachSkill(31221003,0,20); // 다크 바인드
  		cm.teachSkill(31221004,0,30); // 메타모포시스	
		cm.teachSkill(31220005,0,15); // 인피니티 포스
  		cm.teachSkill(31220006,0,10); // 뱀피릭 터치
  		cm.teachSkill(31220007,0,30); // 메이플 용사
  		cm.teachSkill(31221008,0,30); // 어드밴스드 웨폰 마스터리
            cm.warp(1000000,0);cm.changeJob(3122);
	    cm.gainItem(1142344, 1);
	    cm.getPlayer().gainAp(5);
	    cm.addEquip(-10, 1099004, 0, 85, 45, 7, 600, 110); //포스 쉴드
	    cm.fakeRelog();
	    ...
	    cm.dispose();
            ...
            cm.resetStats(35,4,4,4);
            cm.warp(1000000,0);cm.changeJob(3600);
	  	...
	  	cm.teachSkill(30010112,1,1); // 데몬스 퓨리
            ...
            cm.warp(1000000,0);cm.changeJob(3610);
	    cm.gainItem(1142576, 1);
	    cm.addEquip(-10, 1353002, 0, 30, 15, 7, 150, 50); 
	    cm.fakeRelog();
            cm.updateChar();
	    cm.playerMessage(5, "<비밀기관 특수요원> 칭호를 획득 하셨습니다.");
	    //cm.playerMessage(6, "화음곡 : 당신에게 약간의 AP와 SP를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 227) {
            cm.warp(1000000,0);cm.changeJob(3611);
	    cm.gainItem(1142577, 1);
	    cm.getPlayer().gainAp(5);
	    cm.addEquip(-10, 1353003, 0, 60, 30, 7, 350, 80); 
	    cm.fakeRelog();
            cm.updateChar();
	    cm.playerMessage(5, "<자유를 추구하는 자> 칭호를 획득 하셨습니다.");
	    //cm.playerMessage(6, "화음곡 : 당신에게 약간의 AP와 SP를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();
        } else if (selection == 228){
  		cm.teachSkill(36121000,0,30); // 데몬 슬래시 최종 강화 -- 4차스킬 시작
  		cm.teachSkill(36121001,0,30); // 데몬 익스플로젼
  		cm.teachSkill(36121002,0,30); // 데몬 임팩트
  		cm.teachSkill(36121003,0,20); // 데빌 크라이
  		cm.teachSkill(36121004,0,20); // 다크 바인드
  		cm.teachSkill(36120005,0,30); // 메타모포시스	
		cm.teachSkill(36120006,0,15); // 인피니티 포스
  		cm.teachSkill(36121007,0,10); // 뱀피릭 터치
  		cm.teachSkill(36121008,0,30); // 메이플 용사
  		cm.teachSkill(36121009,0,30); // 어드밴스드 웨폰 마스터리
  		cm.teachSkill(36120010,0,30); // 하드 스킨
  		cm.teachSkill(36121013,0,30);
  		cm.teachSkill(36121014,0,30); 
  		cm.teachSkill(36120015,0,30); 
  		cm.teachSkill(36120016,0,30); // 홀로그램 그래피티 : 지원
  		cm.teachSkill(36121011,0,30); // 핀포인트 로켓 최종 강화
  		cm.teachSkill(36121012,0,30); // 멀티래터럴 VI

            cm.warp(1000000,0);cm.changeJob(3612);
	    cm.gainItem(1142578, 1);
	    cm.getPlayer().gainAp(5);
	    cm.addEquip(-10, 1353004, 0, 85, 45, 7, 600, 110);
	    cm.fakeRelog();
            cm.updateChar();
	    cm.playerMessage(5, "<경계에 서있는 자> 칭호를 획득 하셨습니다.");
	    //cm.playerMessage(6, "화음곡 : 당신에게 약간의 AP와 SP를 드렸습니다. 당신이 가는 길에 좋은 일만 있으시길 바랍니다~");
	    cm.dispose();

        } else if (selection == 116) {
            cm.resetStats(35,4,4,4);
            cm.warp(1000000,0);cm.changeJob(6100);
		...
		cm.teachSkill(60001225, 1, 1);
            ...
            cm.warp(1000000,0);cm.changeJob(6110);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(6111);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(6112);
  	...
  	cm.teachSkill(61120013,0,1); // 공격모드 3차 강화
	    ...
	    cm.dispose();
            ...
            if (cm.getPlayer().getGender() == 1) {
            	...
            	cm.warp(1000000,0);cm.changeJob(6500);
		...
		cm.teachSkill(65121008,1,1); // 트리니티 3타
            ...
            cm.warp(1000000,0);cm.changeJob(6510);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(6511);
	    ...
	    cm.dispose();
            ...
            cm.warp(1000000,0);cm.changeJob(6512);
