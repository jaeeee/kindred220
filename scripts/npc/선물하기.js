...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			var ask = "어느 타입의 아이템을 선물하시겠습니까? 드는비용 1억\r\n";
			...
			operation = selection;
			if (operation == 1) {
				type = RaveInventoryType.EQUIP;
				yes = 1;
			} else if (operation == 2) {
				type = RaveInventoryType.USE;
				yes = 2;
			} else if (operation == 4) {
				type = RaveInventoryType.SETUP;
				yes = 4;
			} else if (operation == 3) {
				type = RaveInventoryType.ETC;
				yes = 3;
			} else if (operation == 5) {
				type = RaveInventoryType.CASH;
				yes = 5;
			}
