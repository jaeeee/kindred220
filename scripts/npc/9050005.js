...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.sendGetText("#b#i5041001# #z5041001##k의 갯수를 입력해주세요.\r\n#Cgray#(1개당 350,000 메소)");
	} else if (status == 1) {
	var text = cm.getText();
	cm.sendYesNo("정말 #b"+text+"#k개를 구입하시겠습니까? 가격은 #b#e"+text*350000+" #n#k메소입니다.");
	
} else if (status == 2) {
	var leftslot = cm.getPlayer().getInventory(DbgInventoryType.CASH).getNumFreeSlot();
	var text = cm.getText();
	if (leftslot < text*1/100) {
	cm.sendOk("캐시칸의 공간이 부족합니다. 캐시칸의 공간을 비우시거나 현재 비어있는 공간의 갯수만큼 입력해주시기 바랍니다.\r\n#Cgray#(캐시창 한 칸에는 #z5041001# 100개가 들어갈 수 있으며 캐시창은 총 60개의 공간이 존재합니다.)");
	cm.dispose();
