...
var status = -1;
	...
	cm.sendSimple("왜부르세여?\r\n#L1##b아카이럼 소환#L2#크로스헌터 마스터의 권한\r\n#L3#죽이라는 메세지가 반복될시 클릭<킬올시스템>");
		
		} else if(status == 1) {
		if(selection == 1) {//소환
			if (cm.getMonsterCount(map) > 0) {
			cm.sendOk("모든 몬스터를 전멸시켜야 소환을해드리져");
			cm.dispose();
		} else if (!cm.haveItem(item, count)) {
			cm.sendOk(""+bn+"를 소환하려면 #i"+item+"#"+count+"개가 필요해여");
			cm.dispose();
		} else {    
	        ...
	        cm.sendOk("어?, #b크로스헌터 마스터#k가 아니신데여.");
	        cm.dispose();
	        }
