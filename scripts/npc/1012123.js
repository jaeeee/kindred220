...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	    ...
	    cm.sendSimple("모든 성형과 머리손질은  단풍잎 200개#k로 이용하실 수 있습니다. 원하시는 안드로이드 코디를 해보세요!\r\n\r\n#e#r※ 주의 : 눈 색 변경이나, 성형시 게임이 종료되는 경우는 다른 눈으로 성형을 하시기 바랍니다. (해당 성형에 맞는 눈색깔 이미지가 없을 경우 튕깁니다.)#k\r\n#e<머리색과 눈색은 꼭 검정색으로바꾼뒤 성형해주세요!!>#n#e#n\r\n\r\n#L1##b머리#k#l\r\n#L2##b머리색#k#l\r\n#L3##b얼굴#k#l\r\n#L4##b눈색깔#k#l");
    } else if (status == 1) {
	if (cm.getAndroidGender() == 0) {if (selection == 1) {
        ...
        if (!cm.haveItem(4001126, 200)) {
            cm.sendOk("단풍잎 200개가 필요합니다. 아이템을 제대로 갖고 계신것이 맞는지 확인해 주세요.");
            cm.dispose();
            return;
        }
