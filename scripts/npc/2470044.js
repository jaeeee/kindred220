...
function action(mode, type, selection) {
	...
	if (selection == 0) {
	if (cm.getMeso() >= 1) {
		var Jessica = "정보확인 유저를 입력해주세요. ";
	if (cm.getPlayer().getGender() == 0) {
		Jessica += "아빠! ";
	} else if (cm.getPlayer().getGender() == 1) {
		Jessica += "엄마! ";
	}
		Jessica += "정보확인 유저를 입력해주세요.!";
		...
		cm.sendGetText("정보확인 유저를 입력해주세요.");
		status = 4;

	} else if (selection == 2) {
	if (cm.getMeso() >= 5000000) {
		cm.sendGetText("닉네임을 변경하시게요? 히힛, 그건 아르미가 전문이라구요!");
		status = 3;
	} else {
		cm.sendOk("캐릭터를 삭제하시려면~ 500만 메소가 필요해요! 아르미도 돈 계산은 할 줄 안다구요!");
		cm.dispose();
	}
	}
