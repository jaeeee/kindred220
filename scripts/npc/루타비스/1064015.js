...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
                var rute = "#r#e<루타비스 북쪽 정원 입구>#k#l#n\r\n";
                if(cm.haveItem(4310027,1)) {
                } else {
                }
                rute += "루타비스 북쪽 봉인의 수호자인 #r벨룸#k이 지키고 있는 정원으로 가는 문이다. #r클리어 기록은 노멀의 경우 당일 자정, 카오스의 경우 매주 목요일 자정을 기준으로 초기화 됩니다.#k\r\n";
        ...
        cm.TimeMoveMap(105200400,105200000,900);
        ...
        if (cm.haveItem(4033611,1)) {
        cm.resetMap(105200800);
        cm.TimeMoveMap(105200800,105200000,900);
        cm.gainItem(4033611,-1);
