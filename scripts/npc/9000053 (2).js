/*
 ...
 가그(seunuin)
 kms1.2.183 버전 전직코드를 추가했습니다.
 
 추가하고싶은 직업코드가 있다면 댓글로 남겨주시길바랍니다.
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getKeyValue("startvalue") == "1") {
            ...
            var demon = "당신의 직업을 설정해주세요. 만약 이 스크립트를 끄고 게임을 플레이하시면, 어떠한 직업도 가지지 못할수가 있습니다.";
            ...
            if (cm.getPlayer().getJob() == 1000) {
                ...
                chat += "#L11# 저는 #r나이트 워커#k로 전직하고 싶습니다.\r\n";
            ...
            if (cm.getPlayer().getJob() == 3001) {
                demon += "현재 당신의 직업군은 #b데몬#k 입니다.\r\n";
                demon += "#L26# 저는 #r데몬슬레이어#k로 전직하고 싶습니다.\r\n";
                demon += "#L27# 저는 #r데몬어벤져#k로 전직하고 싶습니다.\r\n";
                cm.sendSimple(demon);
            }
            if (cm.getPlayer().getJob() == 3002) {
                zeno += "현재 당신의 직업은 #b제논#k 입니다.\r\n";
                zeno += "#L28# 저는 #r게임#k을 시작하고 싶습니다.\r\n";
                cm.sendSimple(zeno);
            }
        } else if (cm.getPlayer().getKeyValue("startvalue") == "0") {
            var chat = "온라인에 오신것을 환영합니다. 저희 서버규칙은 아래와 같습니다.\r\n";
            chat += "#r[1]#k #b버그 또는 오류를 발견했을경우 GM에게 문의를 주셔야합니다.#k\r\n";
            chat += "#r[2]#k #b온라인속에서 핵 또는 불법적인 프로그램을 사용할경우 영구벤입니다..#k\r\n";
