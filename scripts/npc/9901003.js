...
function action(mode, type, selection) {
    if (status >= 0 && mode == 0) {
	cm.sendOk("이용해주셔서 감사합니다~~ 생각이 있으시면 또 오세요~");
	cm.dispose();
	return;
    }
    if (mode == 1)
    ...
    if (status == 0) {
        if (cm.getPlayer().getAndroid() == null) {
            cm.sendOk("안녕하세요~ 2기 홍보대사였던 #r거침없는마녀#k입니다, 천외천스토리 오픈 기념으로 저가 안드로이드 관련 코디를 하고 있는데요, 필요하세요?? 모든 성형과 머리손질은 #b#i5155000# #t5155000##k를 이용하여 이용하실 수 있습니다. 원하시는 안드로이드 코디를 해보세요!\r\n\r\n#b안드로이드가 없는 분은 안드로이드 메이크업을 하실 수 없습니다. 안드로이드와 함께 찾아와 주세요.#k");
            cm.dispose();
            return;
        }
        if (cm.getPlayer().getAndroid().getItemId() != 1662002 && cm.getPlayer().getAndroid().getItemId() != 1662003 && cm.getPlayer().getAndroid().getItemId() != 1662006) {
            cm.sendOk("안녕하세요~ 2기 홍보대사였던 #r거침없는마녀#k입니다, 천외천스토리 오픈 기념으로 저가 안드로이드 관련 코디를 하고 있는데요, 필요하세요?? 모든 성형과 머리손질은 #b#i5155000# #t5155000#를 이용하여 이용하실 수 있습니다. 원하시는 안드로이드 코디를 해보세요!\r\n\r\n#b고급형 안드로이드 / 오 마이 프린세스를 소지한 사람에 한해서 안드로이드의 성형과 머리손질이 가능합니다. 보급형 안드로이드의 경우 성형, 머리손질이 불가능합니다.#k");
            cm.dispose();
            return;
        }
	    cm.sendSimple("안녕하세요~ 2기 홍보대사였던 #r거침없는마녀#k입니다, 천외천스토리 오픈 기념으로 저가 안드로이드 관련 코디를 하고 있는데요, 필요하세요?? 모든 성형과 머리손질은 #b#i5155000# #t5155000##k를 이용하여 이용하실 수 있습니다. 원하시는 안드로이드 코디를 해보세요!\r\n\r\n#e#r※ 주의 : 눈 색 변경이나, 성형시 게임이 종료되는 경우는 다른 눈으로 성형을 하시기 바랍니다. (해당 성형에 맞는 눈색깔 이미지가 없을 경우 튕깁니다.)#e#n\r\n\r\n#L1##g헤어 스타일 코디네이팅#k#l       #L2##d헤어 컬러 코디네이팅#k#l\r\n#L3##k아이 라인(얼굴) 코디네이팅#l   #L4##b아이 컬러 코디네이팅#k#l\r\n\r\n#b% 코디 시스템을 이용할 때에는 1회당 #b#i5155000# #t5155000#\r\n#r(캐시샵에서 구매)#b 1개가 필요합니다.#k");
    } else if (status == 1) {
	if (cm.getAndroidGender() == 0) {if (selection == 1) {
		...
		cm.askAvatarAndroid("당신의 안드로이드에게 원하는 패션을 선택해주세요.", hairnew);
	    } else if (selection == 2) {
		beauty = 3;
		haircolor = Array();
		...
		cm.askAvatarAndroid("당신의 안드로이드에게 원하는 패션을 선택해주세요.", haircolor);
	    } else if (selection == 3) {
		beauty = 4;
		facenew = Array();
		...
		cm.askAvatarAndroid("당신의 안드로이드에게 원하는 패션을 선택해주세요.", facenew);
	    } else if (selection == 4) {
		beauty = 5;
		...
		cm.askAvatarAndroid("당신의 안드로이드에게 원하는 패션을 선택해주세요.", colors);
	    }
	} else {
            if (selection == 1) {
		beauty = 2;
        ...
        if (!cm.haveItem(5155000, 1)) {
            cm.sendOk("#b#i5155000# #t5155000##k가 필요합니다. 아이템을 제대로 갖고 계신것이 맞는지 확인해 주세요.");
            cm.dispose();
            return;
        }
	...
	cm.playerMessage(5, "카르타의 진주를 1개를 사용하여 1회 안드로이드 코디네이팅을 완료 하였습니다.");
        cm.sendOk("성형과 머리손질이 완료되었습니다~ 거울을 한번 봐 주시는걸 추천해드릴게요~ 즐거운 하루 되세요!\r\n천외천팩, GDN Project, 천외천스토리의 창시자 #b가군님#k을 많이 응원해주세요!");
