...
function action(mode, type, selection) {
	...
	var leaf = cm.itemQuantity(4310058);
        var trade = "러브리스 아이템 상점입니다. 어떤 아이템을 원하세요?\r\n러브리스 아이템은 #b[매그너스 코인]#k으로 교환하실수 있습니다. #b[러브리스 코인]#k은 마을에 있는 #b[문 교수]#k가 특정아이템을 구해오면 준다더군요..\r\n";
        ...
        cm.sendYesNo("환생포인트 3500을 #b#i"+itemsArray[select][1]+"# #z"+itemsArray[select][1]+"#으로 교환 하시겠습니까?");
    } else if (status == 3) {
        var itemsArray = getArray(menuSelect);
        if (cm.getPlayer().getFame()>=3500 && cm.canHold(itemsArray[select][1])) {
            cm.addFame(-3500);
