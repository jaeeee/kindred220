...
function ConstructModeSelectionString() {
    ...
    for (var i = 0;i < Modes.length;i++) {
        ret += "#L"+i+"#"+Modes[i][0]+" 지뢰 "+Modes[i][3]+"개,  ("+Modes[i][1]+"x"+Modes[i][2]+")#l\r\n";
    }
    return ret;
}
...
function ConstructBoardString() {
    var ret = "#d옆에 표시된만큼 찾으세요. (지뢰 말고)#k ->#k #r" + LeftToBeRevealed + "\r\n";
    var num = 0;
    for (var i = 0;i < NumCols;i++) {
        for (var j = 0;j < NumRows;j++) {
...
function action(mode, type, selection) {
    if (mode == -1 || mode == 0)
        ...
        if (status == 0) {
            cm.sendSimple("#e게임의 난이도를 선택하십시오.#n\r\n"+ConstructModeSelectionString())
        } else if (status == 1) {
            ...
            if (Board[Move[0]][Move[1]] == MINE_ITEM) {
                //Lose
                cm.sendOk(Header() + ConstructEndBoardString() + "\r\n게임에서 졌습니다. 다음 번에는 꼭 승리하시기 바랍니다.\r\n시간: " + GetGameTime((new Date()).getTime()));
                cm.dispose();
            } else {
                if (IsGameOver()) {
