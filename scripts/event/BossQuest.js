...
function playerExit(eim, player) {
    ...
    player.dropMessage(6, "[The Boss Quest] 클리어한 스테이지 수 만큼 포인트가 저장되었습니다.");
...
function allMonstersDead(eim) {
	...
	map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[The Boss Quest] 축하합니다! 모든 스테이지를 클리어하셨습니다. < 총 포인트 " + points + " >"));
	map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[The Boss Quest] 모든 스테이지를 클리어하여 자유시장으로 돌아갑니다."));
	disbandParty();
    } else {
	map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[The Boss Quest] 스테이지를 클리어하셨습니다. 현재 포인트는 " + points + "입니다. 10초후에 다음 스테이지 보스몬스터가 소환됩니다."));
...
function beginQuest(eim) {
    var map = eim.getMapInstance(mapid, 0);
    map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[The Boss Quest] 어둠의 생명체가 다가오고있습니다. 30초남았습니다."));
