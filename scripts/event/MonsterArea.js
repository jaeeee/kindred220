...
function announce(eim) {
    var text = "[몬스터 아레나] 잠시후에 몬스터가 소환됩니다.";
        eim.getMapFactory().getMap(950200100).broadcastMessage(MainPacketCreator.showEffect("UlWindowBT.img/MonsterBattle/Darw"));//UlWindowBT.img
...
function summon(eim) {
    ...
    for (var i = 0 ; i < 10 ; i++) {
        var mob = BelowLifeProvider.getMonster(mobid);
        eim.registerMonster(mob);
	    var modified = em.newMonsterStats();
	    modified.setOExp(mob.getMobExp() * 4);
...
function allMonstersDead(eim) {
 ...
 if (level >= 1 && level <= 5) {
        ...
        eim.getMapFactory().getMap(950200100).broadcastMessage(MainPacketCreator.showEffect("UI/UIWindowBT.img/MonsterBattle/Darw"));//ULWindowBT.img
	...
	for (var i = 0; i < party1.size(); i++) {
		party1.get(i).gainItem(4310016, 1, false, -1, "빨간단추 흭득");
...
var chr = eim.getPlayers().iterator().next();
     ...
     if (party == null) {
     chr.gainItem(4310016, 1, false, -1, "몬스터 아레나");      
     ...
     var chr1 = toRem.next();
     chr1.gainItem(4310016, 1, true, -1, "몬스터 아레나");      
     eim.unregisterPlayer(chr);
     var exit = em.getChannelServer().getMapFactory().getMap(100000000);
