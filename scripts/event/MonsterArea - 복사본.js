...
function playerEntry(eim, player) {
    ...
    var mapzz = eim.getMapFactory().getMap(932100000);
...
function announce(eim) {
    var text = "[몬스터 아레나] 잠시후에 몬스터가 소환됩니다.";
        eim.getMapFactory().getMap(932100000).broadcastMessage(MainPacketCreator.showEffect("UlWindowBT.img/MonsterBattle/Darw"));//UlWindowBT.img
    eim.getMapFactory().getMap(932100000).broadcastMessage(UIPacket.showInfo(text));
    eim.getMapFactory().getMap(932100000).broadcastMessage(MainPacketCreator.getGMText(7, text));
...
function summon(eim) {
    ...
    eim.getMapFactory().getMap(932100000).broadcastMessage(UIPacket.showInfo(text));
    eim.getMapFactory().getMap(932100000).broadcastMessage(MainPacketCreator.getGMText(5, text)); //-109,154
    var mobid = 0;
    var level = Integer.parseInt(eim.getProperty("level"));
    eim.getMapFactory().getMap(932100000).broadcastMessage(UIPacket.showWZEffect("UI/tutorial.img/20", 1));
    ...
    for (var i = 0 ; i < 10 ; i++) {
        ...
        eim.getMapFactory().getMap(932100000).spawnMonsterOnGroundBelow(mob, new java.awt.Point(1870, -1347));
    }
    for (var i = 0 ; i < 10 ; i++) {
       var mob = MapleLifeProvider.getMonster(mobid);
        eim.registerMonster(mob);
